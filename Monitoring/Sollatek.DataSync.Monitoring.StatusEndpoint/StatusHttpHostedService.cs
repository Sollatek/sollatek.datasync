...
public sealed class StatusHttpHostedService : IHostedService, IAsyncDisposable
{
    ...
    private static object ToResponse(SyncRunStatus status)
    {
        return new
        {
            ...
            status.ScheduleMode,
            status.NextRunAtUtc,
            status.PlannedRangeEndUtc,
            status.ExpectedCompletedRangeEndUtc,
            status.CurrentRangeStartUtc,
            status.CurrentRangeEndUtc,
            status.LastCompletedRangeEndUtc,
            status.LagSeconds,
            status.LagPeriods,
            status.PlannedEntityCount,
            status.StartedAt,
            ...
            status.TotalAllocatedBytes,
            status.WorkingSetBytes,
            status.PrivateMemoryBytes,
            status.PeakWorkingSetBytes,
            status.AsyncExportsPending,
            status.AsyncExportsPolling,
            status.AsyncExportsDownloaded,
            status.AsyncExportsProcessing,
            status.AsyncExportsFailed,
            status.AsyncExportsExpired
        };
    }
}
